    ...
    if(username){
        element = <>
            <li className="nav-item" id="nav_avatar">
                <div ><img src={avatar} className="avatar" /></div>
                <p style={{ color: "black", height: 40, marginTop: 20, marginLeft: 10 }}>{username}</p>
            </li>
            <li >
                <button type="button" class="btn btn-primary" style={{marginTop:'17px', marginLeft:'10px'}} onClick={handleLogOut}>Log Out</button>
            </li>
        </>
    } else {
        element = <>
